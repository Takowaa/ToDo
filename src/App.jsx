...
function App() {
  ...
  const createItem = () => {
    axios
      .post(`https://656d7f61bcc5618d3c23460f.mockapi.io/api/todos`, {
        title: value,
        isCompleted: false,
      })
  ...
  const deleteItem = id => {
    axios
      .delete(`https://656d7f61bcc5618d3c23460f.mockapi.io/api/todos/${id}`)
      .then(() => {
  ...
  const handleUpdateTodo = todo => {
    axios
      .put(
        `https://656d7f61bcc5618d3c23460f.mockapi.io/api/todos/${todo.id}`,
        todo,
      )
      .then(({ data }) => {
        const updateList = todos.map(elem =>
  ...
  useEffect(() => {
    axios('https://656d7f61bcc5618d3c23460f.mockapi.io/api/todos').then(
      ({ data }) => setTodos(data),
    );
  }, []);
  return (
